...
int main(int argc, char * argv[]) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW); // 把 indices 复制到缓冲中
    
    // then configure Vertex-Attributes
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0); // 链接顶点属性到 OpenGL
    glEnableVertexAttribArray(0); // 启用顶点属性
    
    // note that this is allowed, the call to glVertexAttribPointer registered VBO as the vertex attribute's bound VBO so afterwards we can safely unbin
